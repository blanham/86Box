...
void
rage128_host_data_finish(rage128_t *dev)
{
    rage128_host_data_t *host = &dev->host_data;
    ...
    if (!host->complete)
        rage128_warn_unsupported(&dev->warned_host_early_end,
                                 "PM4 HOSTDATA_BLT ended an incomplete transfer",
                                 (host->row << 16) | host->col);
    host->active = 0;
}
...
rage128_accel_reg_written(rage128_t *dev, uint32_t reg)
{
    ...
    switch (reg) {
        ...
        case R128_DP_GUI_MASTER_CNTL:
            /*
             * XAA carries this command bit in its baseline master word and
             * programs CLR_CMP_CNTL only for transparent operations.
             */
            if (value & R128_GMC_CLR_CMP_CNTL_DIS)
                R128_REG(dev, R128_CLR_CMP_CNTL) = 0;
